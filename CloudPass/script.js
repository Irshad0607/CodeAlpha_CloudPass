const API = 'https://cloudpass-api-aabfe9cueqdhd8fg.centralindia-01.azurewebsites.net/api';
    ...
    container.innerHTML = data.map(pass => `
      <div class="pass-card">
        ...
        <div class="pass-grid">
          ...
          <div><span>Pass Type</span><strong>${capitalize(pass.passType)}</strong></div>
          <div><span>Price</span><strong>₹${pass.price}</strong></div>
          <div><span>Issued On</span><strong>${new Date(pass.createdAt).toLocaleDateString()}</strong></div>
        </div>
